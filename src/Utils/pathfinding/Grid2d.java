...
public class Grid2d {
	...
	public class MapNode implements Node<MapNode> {
		...
		public double getTraversalCostLegacy(MapNode neighbour) {
			return 1 + map[neighbour.y][neighbour.x];
		}
		public double getTraversalCost(MapNode neighbour) {
			double z1 = map[y][x];
			double z2 = map[neighbour.y][neighbour.x];
			if(z2<0) {
				return Double.POSITIVE_INFINITY;
			}else
			{
				double delta = Math.abs(z2-z1);
				double zcost=Math.max(delta-1,0)*0.1;
				double altcost = (Math.exp(z2/1000)-1)*0.4;
				return 1 + zcost+altcost;
			}
		}
